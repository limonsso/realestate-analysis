...
try:
    ...
    def ensure_directories():
        for directory in [INPUT_DIR, OUTPUT_DIR, CLEANED_DATA_DIR, REPORTS_DIR, LOGS_DIR]:
...
class RealEstateDataCleaner:
    ...
    def _consolidate_redundant_columns(self):
        ...
        if len(existing_revenue_cols) > 1:
            ...
            try:
                ...
                for col in existing_revenue_cols:
                    if col in self.df_cleaned.columns:
                        if not isinstance(self.df_cleaned[col], pd.Series):
                            ...
                            # Forcer la conversion en Series de manière plus robuste
                            try:
                                if hasattr(self.df_cleaned[col], 'values'):
                                    if hasattr(self.df_cleaned[col].values, 'flatten'):
                                        ...
                                        flattened_values = self.df_cleaned[col].values.flatten()
                                        # Prendre seulement les premières valeurs pour correspondre à la longueur
                                        if len(flattened_values) > len(self.df_cleaned):
                                            flattened_values = flattened_values[:len(self.df_cleaned)]
                                        self.df_cleaned[col] = pd.Series(flattened_values, index=self.df_cleaned.index)
                                    else:
                                        # DataFrame simple
                                        self.df_cleaned[col] = pd.Series(self.df_cleaned[col].values, index=self.df_cleaned.index)
                                else:
                                    # Autre type d'objet
                                    self.df_cleaned[col] = pd.Series(self.df_cleaned[col], index=self.df_cleaned.index)
                            except Exception as conv_error:
                                logger.warning(f"    Erreur lors de la conversion directe: {conv_error}")
                                # Fallback: essayer de récupérer la première colonne
                                if hasattr(self.df_cleaned[col], 'iloc'):
                                    try:
                                        first_col = self.df_cleaned[col].iloc[:, 0]
                                        if len(first_col) >= len(self.df_cleaned):
                                            self.df_cleaned[col] = first_col[:len(self.df_cleaned)]
                                        else:
                                            # Étendre avec des valeurs NaN
                                        ...
                                        # Dernier recours: créer une colonne vide
                                        self.df_cleaned[col] = pd.Series([pd.NA] * len(self.df_cleaned), index=self.df_cleaned.index)
                                        logger.warning(f"    Colonne {col} remplacée par des valeurs NA")
                
                # Créer la colonne consolidée de manière plus robuste
                consolidated_values = []
                for idx in range(len(self.df_cleaned)):
                    row_sum = 0
                    for col in existing_revenue_cols:
                        try:
                            val = self.df_cleaned.loc[idx, col]
                ...
                self.df_cleaned['revenue_consolidated'] = consolidated_values
    ...
    def _clean_physical_characteristics(self):
        ...
        try:
            ...
            for col in bedroom_cols:
                if col in self.df_cleaned.columns:
                    ...
                    try:
                            ...
                            try:
                                if hasattr(self.df_cleaned[col], 'values'):
                                    if hasattr(self.df_cleaned[col].values, 'flatten'):
                                        ...
                                        # Prendre seulement les premières valeurs pour correspondre à la longueur
                                        if len(values) > len(self.df_cleaned):
                                            values = values[:len(self.df_cleaned)]
                                    else:
                                        # DataFrame simple
                                ...
                                if hasattr(self.df_cleaned[col], 'iloc'):
                                    try:
                                        ...
                                        if len(first_col) >= len(self.df_cleaned):
                                            ...
                                            if len(first_col) > len(self.df_cleaned):
                                                first_col = first_col[:len(self.df_cleaned)]
                                            # Créer une nouvelle Series
                        ...
                        # Vérifier à nouveau après conversion et forcer la conversion si nécessaire
                        if not isinstance(self.df_cleaned[col], pd.Series):
                            logger.error(f"    Échec de la conversion de {col} en Series")
                            # Forcer la conversion en Series de manière directe
                            try:
                                # Créer une nouvelle Series directement
                                new_series = pd.Series([pd.NA] * len(self.df_cleaned), index=self.df_cleaned.index)
                                # Remplacer la colonne dans le DataFrame
                                self.df_cleaned = self.df_cleaned.drop(columns=[col])
                                self.df_cleaned[col] = new_series
                                logger.info(f"    Colonne {col} forcée en Series avec valeurs NA")
                            except Exception as force_error:
                                logger.error(f"    Erreur lors du forçage de {col}: {force_error}")
                                # Dernier recours: supprimer la colonne problématique
                                self.df_cleaned = self.df_cleaned.drop(columns=[col])
                                logger.warning(f"    Colonne {col} supprimée car impossible à convertir")
                        
                        # Vérifier le type et afficher les valeurs de manière sécurisée
                        try:
                            if isinstance(self.df_cleaned[col], pd.Series):
                                logger.info(f"    Valeurs de {col} avant conversion: {self.df_cleaned[col].head().tolist()}")
                            else:
                                logger.info(f"    Type de {col}: {type(self.df_cleaned[col])}")
                                if hasattr(self.df_cleaned[col], 'head'):
                                    logger.info(f"    Premières valeurs de {col}: {self.df_cleaned[col].head()}")
                        except Exception as log_error:
                            logger.warning(f"    Impossible d'afficher les valeurs de {col}: {log_error}")
                        ...
                        # Afficher les valeurs de manière sécurisée
                        try:
                            if hasattr(self.df_cleaned[col], 'head'):
                                if isinstance(self.df_cleaned[col], pd.Series):
                                    logger.error(f"    Valeurs de {col}: {self.df_cleaned[col].head().tolist()}")
                                else:
                                    logger.error(f"    Valeurs de {col}: {self.df_cleaned[col].head()}")
                            else:
                                logger.error(f"    Pas de méthode head pour {col}")
                        except Exception as log_error:
                            logger.error(f"    Impossible d'afficher les valeurs de {col}: {log_error}")
                        raise
            ...
            if 'construction_year' in self.df_cleaned.columns:
                ...
                try:
                    # Vérifier et corriger le type de la colonne
                    if not isinstance(self.df_cleaned['construction_year'], pd.Series):
                        logger.warning(f"    Colonne construction_year n'est pas une Series: {type(self.df_cleaned['construction_year'])}")
                        # Forcer la conversion en Series de manière directe
                        try:
                            # Créer une nouvelle Series directement
                            new_series = pd.Series([pd.NA] * len(self.df_cleaned), index=self.df_cleaned.index)
                            # Remplacer la colonne dans le DataFrame
                            self.df_cleaned = self.df_cleaned.drop(columns=['construction_year'])
                            self.df_cleaned['construction_year'] = new_series
                            logger.info(f"    Colonne construction_year forcée en Series avec valeurs NA")
                        except Exception as force_error:
                            logger.error(f"    Erreur lors du forçage de construction_year: {force_error}")
                            # Dernier recours: supprimer la colonne problématique
                            self.df_cleaned = self.df_cleaned.drop(columns=['construction_year'])
                            logger.warning(f"    Colonne construction_year supprimée car impossible à convertir")
                            return
    ...
    def _remove_duplicates(self):
        ...
        # Réinitialiser l'index pour éviter les problèmes de doublons
        self.df_cleaned = self.df_cleaned.reset_index(drop=True)
        
        # Vérifier et corriger les colonnes dupliquées
        duplicate_columns = self.df_cleaned.columns[self.df_cleaned.columns.duplicated()].tolist()
        ...
        logger.info(f"✅ Index réinitialisé: {len(self.df_cleaned)} propriétés")
        logger.info(f"✅ Colonnes finales: {len(self.df_cleaned.columns)}")
    ...
    def _export_data(self):
        ...
        parquet_file = CLEANED_DATA_DIR / f"{base_filename}.parquet"
        ...
        if 'longitude' in self.df_cleaned.columns and 'latitude' in self.df_cleaned.columns:
            geojson_file = CLEANED_DATA_DIR / f"{base_filename}.geojson"
    ...
    def _create_quality_report(self):
        ...
        self.quality_report = {
            ...
            'dataset_info': {
                'total_records': int(len(self.df_cleaned)),
                'total_columns': int(len(self.df_cleaned.columns)),
                'memory_usage_mb': float(self.df_cleaned.memory_usage(deep=True).sum() / 1024**2)
            },
            'data_quality': {
                'completeness_score_mean': float(self.df_cleaned['completeness_score'].mean()) if 'completeness_score' in self.df_cleaned.columns else 0.0,
                'missing_data_percentage': float((self.df_cleaned.isnull().sum().sum() / (len(self.df_cleaned) * len(self.df_cleaned.columns))) * 100)
            },
        ...
        import json
        ensure_directories()  # S'assurer que les dossiers existent
        ...
        with open(report_file, 'w', encoding='utf-8') as f:
            json.dump(self.quality_report, f, indent=2, ensure_ascii=False, cls=NumpyEncoder)
